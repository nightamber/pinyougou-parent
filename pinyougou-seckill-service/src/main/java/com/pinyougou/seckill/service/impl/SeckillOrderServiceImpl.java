...
public class SeckillOrderServiceImpl implements SeckillOrderService {
	...
	public void submitOrder(Long seckillId, String userId) {
		...
		if(seckillGoods.getStockCount()<= 0){
			throw new RuntimeException("商品已经抢购一空");
		}
		//减少库存
		seckillGoods.setStockCount(seckillGoods.getStockCount()-1);
		redisTemplate.boundHashOps("seckillGoods").put(seckillId,seckillGoods);
		if(seckillGoods.getStockCount()==0){//商品被秒光
			seckillGoodsMapper.updateByPrimaryKey(seckillGoods);//同步数据库
	...
	public void deleteOrderFromRedis(String userId, Long orderId) {
		...
		if(seckillOrder!=null && seckillOrder.getId().longValue()== orderId.longValue() ){
			redisTemplate.boundHashOps("seckillOrder").delete(userId);//删除缓存的订单
			//库存增加
			//1.从缓存中提取秒杀商品
